...
hatchling picks it up via ``packages = ["src/sim_plugin_simpy"]``
...
def test_wheel_contains_skills(tmp_path: Path) -> None:
    ...
    wheels = list(out_dir.glob("sim_plugin_simpy-*.whl"))
    ...
    required = {
        "sim_plugin_simpy/__init__.py",
        "sim_plugin_simpy/driver.py",
        "sim_plugin_simpy/_skills/simpy/SKILL.md",
    }
